...
def companies_sorting_filtring(avg_grade, sort_by, sort_dir, has_grades, query=""):
    ...
    if query is not None or query != "":
        if sort_by == 'alphabetically' and sort_dir == 'ASC':
            results = Companies.objects.filter(Q(name__icontains=query) &
                                               Q(status="accepted") &
                                               Q(pk__in=avg_ratings)).order_by('-name')
            ...
            results = Companies.objects.filter(Q(name__icontains=query) &
                                               Q(status="accepted") &
                                               (Q(pk__in=avg_ratings) if has_grades else Q())) \
